# -*- coding: utf-8 -*-
"""CasualSelfAttention module for Decoder-Only Transformer.
...
class CasualSelfAttention(torch.nn.Module):
  ...
  def __init__(self, embedding_dim: int, num_heads: int, max_token_length: int, bias: bool=False, dropout: float=0.0):
    ...
    self.c_attn = torch.nn.Linear(embedding_dim, 3*embedding_dim, bias=bias)

    #projection of concatenated attention head outputs
    self.c_proj = torch.nn.Linear(embedding_dim, embedding_dim, bias=bias)
